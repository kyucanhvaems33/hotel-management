...
namespace QLKS
{
    public partial class F_QLKH : Form
    {
        ...
        {
            string query = "SELECT ( CASE WHEN NOT EXISTS(SELECT NULL FROM DichVu) THEN 1 ELSE (SELECT IDENT_CURRENT('KhachHang') + 1) END) AS cl1";
            DataTable tb = GetData.getdata(query, false, false);
            tb_makh.Text = tb.Rows[0][0].ToString();
        }
