...
namespace QLKS
{
    public partial class F_LogIn : Form
    {
        ...
        private void layTK()
        {
            string query = "SELECT username,pass,TK.MaNV,Quyen,NhanVien.HoTen FROM TK,NhanVien WHERE username = @user AND pass = @pass AND TK.MaNV = NhanVien.MaNV";
            GetData.con.Open();
            GetData.com = new SqlCommand(query, GetData.con);
            GetData.com.Parameters.Add("@user", txt_User.Text);
            GetData.com.Parameters.Add("@pass", txt_Pass.Text);
        ...
        private void putlogintime()
        {
            string query = "insert into listlogintime (username,login,logout) values (@user,@login,@logout)";
            GetData.con.Open();
            GetData.com = new SqlCommand(query, GetData.con);
            GetData.com.Parameters.Add("@user", TK.user);
            GetData.com.Parameters.Add("@login", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss", new CultureInfo("vi-VN")));
            GetData.com.Parameters.Add("@logout", DBNull.Value);
            GetData.putdata();
        }
